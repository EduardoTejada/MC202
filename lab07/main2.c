...
int main(void) {
    ...
    while (1) {
        ...
        if (strcmp(cmd, "c") == 0) {
            ...
            head = NULL; // Define a lista como vazia
        }

        // Comando para inserir um nó na posição p
        else if (strcmp(cmd, "i") == 0) {
            scanf("%d %lf", &p, &x);
            head = insertNode(head, p, x); // Insere o nó na posição especificada
        }

        // Comando para remover o nó na posição p
        else if (strcmp(cmd, "r") == 0) {
            scanf("%d", &p);
            head = removeNode(head, p); // Remove o nó na posição especificada
        }

        // Comando para imprimir a lista
        else if (strcmp(cmd, "p") == 0) {
            listPrint(head); // Imprime todos os nós da lista
        }

        // Comando para reverter trecho da lista de i até t
        else if (strcmp(cmd, "v") == 0) {
            scanf("%d %d", &i, &t);
            head = reverseList(head, i, t); // Reverte o trecho especificado da lista
        }

        // Comando para mover trecho da lista de i até t para posição p
        else if (strcmp(cmd, "x") == 0) {
            scanf("%d %d %d", &i, &t, &p);
            head = moveList(head, i, t, p); // Move o trecho especificado para a nova posição
        }

        // Comando para terminar o programa e liberar a memória
        else if (strcmp(cmd, "t") == 0) {
            listFree(head); // Libera a memória da lista
            break; // Termina o loop e encerra o programa
        }
    }
    return 0;
}
