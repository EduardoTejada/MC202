...
CSR* inicializaCSR(int nlinhas, int ncols, int nnz) {
    if (nlinhas <= 0 || ncols <= 0 || nnz <= 0 || nlinhas > INT_MAX || nnz > INT_MAX) {
        return NULL;
    }
...
int getValor(CSR *matriz, int linha, int col) {
    ...
    for (int i = matriz->linha_ptr[linha]; i < matriz->linha_ptr[linha + 1]; i++) {
...
int main() {
    ...
    for (int i = 0; i < k; i++) {
        scanf("%d %d %d", &elementos[i].linha, &elementos[i].col, &elementos[i].valor);
        if (elementos[i].linha > max_linha) max_linha = elementos[i].linha;
        if (elementos[i].col > max_col) max_col = elementos[i].col;
    }
